defmodule BrainWall.Scorer do
  def dislikes(hole, pose) do
    for h <- hole do
      candidates = (for p <- pose, do: {h,p}) |> Enum.filter(fn x -> !intersects?(edges(hole),x) end)
  ...
  def intersects?(e, {[hx,hy],[px,py]}) do
    e |> Enum.any?(fn {[x1,y1],[x2,y2]} -> {_,t,_} = SegSeg.intersection({x1,y1}, {x2,y2}, {hx,hy}, {px,py});t == :interior end)
  end
end
